<template>
  <div class="min-h-screen bg-gradient-to-r from-indigo-600 to-blue-500">
    ...
    <div class="flex items-start">
      <!-- Sidebar -->
      <aside class="w-64 bg-gradient-to-r from-indigo-600 to-blue-500 bg-opacity-20 backdrop-blur-md shadow-md p-6 hidden md:flex flex-col space-y-8">
        ...
        <form v-else @submit.prevent="submitQuiz" class="w-full max-w-3xl space-y-6">
          <transition-group name="slide-fade" tag="div">
            <div v-for="(quiz, index) in quizList" :key="index"
                 class="p-6 bg-gradient-to-r from-indigo-700 to-purple-800 bg-opacity-20 backdrop-blur-md rounded-xl shadow transition transform hover:scale-105 hover:shadow-2xl hover:from-indigo-500 hover:to-purple-500 cursor-pointer">
              <p class="font-semibold text-white text-lg">{{ index + 1 }}. {{ quiz.question }}</p>
              <div class="mt-3 space-y-2">
                <label v-for="(option, i) in quiz.options" :key="i"
                       class="flex items-center space-x-3 cursor-pointer text-white">
                  <input type="radio" :name="'question' + index" :value="option" v-model="answers[index]" required
          ...
          <button type="submit"
                  class="w-full bg-indigo-500 hover:bg-indigo-600 text-white py-3 rounded-xl text-lg font-semibold shadow">
            Submit Quiz
          </button>
        </form>
